...
/** Injected measurement context supplied by the host runtime. */
let injectedMeasureContext = null;
...
/**
 * Inject a host-provided measurement context.
 *
 * The injected object must expose the subset of CanvasRenderingContext2D
 * that Pretext actually uses:
 *   - measureText(text: string): { width: number }
 *   - font: string (settable)
 *
 * When set, this bypasses the built-in OffscreenCanvas / DOM canvas
 * creation entirely, enabling the engine to run on QuickJS, Hermes,
 * Node (via node-canvas), or any host that can supply text metrics.
 *
 * Call once before any layout pass. Re-calling with the same context
 * is a no-op; calling with a different context resets internal caches.
 */
export function setMeasureContext(ctx) {
    if (ctx === injectedMeasureContext)
        return;
    injectedMeasureContext = ctx;
    // Caches are tied to the previous context's font metrics — clear them.
    clearMeasurementCaches();
}
export function getMeasureContext() {
    if (injectedMeasureContext !== null)
        return injectedMeasureContext;
    ...
    throw new Error('Text measurement requires OffscreenCanvas, a DOM canvas context, or an injected MeasurementContext. ' +
        'Pass one via setMeasureContext() before calling the layout engine.');
}
...
function getEmojiCorrection(font, fontSize) {
    ...
    correction = 0;
    // Emoji correction compares canvas width against DOM width to detect
    // browser-specific emoji inflation (e.g., Chrome on macOS). On hosts
    // without a DOM (QuickJS, Node), we skip the correction — the canvas
    // metric is the best available approximation.
